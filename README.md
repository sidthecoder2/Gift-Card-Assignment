...
- **Vendor list/detail endpoints are genuinely mocked, not just seeded.** `VendorClient` has `listCards()` and `getCardDetail()` methods on each mock implementation, hardcoded to return JSON shaped exactly like the assignment's QwikGift/GiftBazaar specs. A `VendorSyncService` calls these at application startup and upserts the merged results into `gift_cards`/`gift_card_vendor_offers` — so the "mock these responses" requirement is satisfied by actual mocked HTTP-shaped methods being called, not by directly seeding pre-merged rows into the database.
